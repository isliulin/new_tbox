...
int SqliteDatabase::initDatabase()
{
	...
	if(flag == 0)
	{
		...
		//create table
		sql = sqlite3_mprintf("create table %s(id INTEGER primary key, data blob, dataLen INTEGER, dev INTEGER, isSend INTEGER)", TABLE_NAME);
		printf("sql:%s\n", sql);
...
* Date          : 2018.01.18
*****************************************************************************/
int SqliteDatabase::insertData(int id, unsigned char *pData, int len, int dev, int isSend)
{
	int ret;
	sqlite3_stmt *stmt;
	char *sql = sqlite3_mprintf("insert into %s values(%d,?, %d, %d, %d)", TABLE_NAME, id, len, dev, isSend);
//	printf("%s\n", sql);
...
int SqliteDatabase::updataisSend(int isSend, int id)
{
	int ret;
	sqlite3_stmt *stmt;
...
unsigned int SqliteDatabase::queryMaxdevID()
{
	int ret;
	unsigned int dev;
	sqlite3_stmt *stmt;

	char *sql = sqlite3_mprintf("select max(dev) from %s", TABLE_NAME);
//	printf("%s\n", sql);
...
int SqliteDatabase::queryMaxDevandID(int &id, unsigned int &dev)
{
	int ret;
	char *sql = NULL;
	sqlite3_stmt *stmt;
	...
	sql = sqlite3_mprintf("select max(id) from %s where dev = %d", TABLE_NAME, dev);
//	printf("%s\n", sql);
...
* Date          : 2018.01.18
*****************************************************************************/
unsigned int SqliteDatabase::queryMinisSendID()
{
	int ret;
	unsigned int id;
	sqlite3_stmt *stmt;

	char *sql = sqlite3_mprintf("select min(id) from %s where id = (select id from %s where isSend = 1)", TABLE_NAME, TABLE_NAME);
//	printf("%s\n", sql);
...
int SqliteDatabase::readData(unsigned char *pBuff, int *len, int id)
{
	...
	char *sql = sqlite3_mprintf("select data from %s where id = %d", TABLE_NAME, id);
//	printf("%s\n", sql);
