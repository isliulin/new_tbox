...
void tboxsignal_log(uint8_t signal)
{
	...
	nLen1 = sprintf(szLine, "tbox signal is %d on \n[%04d-%02d-%02d %02d:%02d:%02d]\n",signal, p_tm->tm_year+1900, p_tm->tm_mon,  p_tm->tm_mday,  p_tm->tm_hour,  p_tm->tm_min, p_tm->tm_sec);
	  ...
	  case SIMCOM_EVENT_DATACALL_DISCONNECTED_IND:	//new sdk interface
            {
				...
				if(datacall_info.status == DATACALL_CONNECTED)
				{
                    ...
                    if(use_dns)
                    {
						...
						ret = query_ip_from_dns(TSP_ADDR_STRING, datacall_info.pri_dns_str ,/*datacall_info.pri_dns_str*/datacall_info.sec_dns_str , target_ip_new);
						
    					if(ret != 0)
    					{
    						printf("query ip fail\n");
							ret = query_ip_from_dns(TSP_ADDR_STRING, (char*)"114.114.114.114" ,(char*)"114.114.114.114" , target_ip_new);
							if(ret != 0)
							{
								printf("query ip fail\n");
								break;
