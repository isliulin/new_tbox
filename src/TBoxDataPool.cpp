...
int TBoxDataPool::setPara(TBoxDataId id, void *pPara, uint32_t size)
{
	switch (id)
	{
		...
		case PEPSCommunicationDetectionTime_ID:
			...
			break;
		case BleAutoUpDateTime_ID:
			TBoxPara.detectionTime.BleAutoUpDateTime = *(uint16_t *)pPara;
			break;
		case GL_DEBUGLOG_SWITCH_ID:
			TBoxPara.dataPara.glDebugLogSwitch = *(uint8_t *)pPara;
			break;
		case TBOX_TRCUKLOADING_ID:
			TBoxPara.dataPara.TboxTrcukLoading = *(uint8_t *)pPara;
			break;
		case TBOX_LATITUDE_ID:
			TBoxPara.dataPara.Tboxlatitude = *(uint32_t *)pPara;
			break;
		case TBOX_LONGITUDE_ID:
			TBoxPara.dataPara.Tboxlongitude = *(uint32_t *)pPara;
			break;
		default:
			break;
	}
...
int TBoxDataPool::getPara(TBoxDataId id, void *pPara, uint32_t size)
{
	...
	{
		case wifi_APMode_Name_Id:
            memcpy(pPara, TBoxPara.dataPara.wifiAPModeName, sizeof(TBoxPara.dataPara.wifiAPModeName));
			break;
		case wifiAPModePassswordId:
            memcpy(pPara, TBoxPara.dataPara.wifiAPModePasssword, sizeof(TBoxPara.dataPara.wifiAPModePasssword));
			break;
		case wifiStaModeNameId:
		...
		case SIM_ICCID_INFO:
			...
			break;
		case CIMI_INFO:
			memcpy(pPara, TBoxPara.dataPara.cimi, sizeof(TBoxPara.dataPara.cimi));
			break;
		case E_CALL_ID:
			memcpy(pPara, TBoxPara.dataPara.E_CALL, sizeof(TBoxPara.dataPara.E_CALL));
			break;
		case B_CALL_ID:
			memcpy(pPara, TBoxPara.dataPara.B_CALL, sizeof(TBoxPara.dataPara.B_CALL));
			break;
		case SERVER_IP_PORT_ID:
		...
		case PEPSCommunicationDetectionTime_ID:
			*(uint16_t *)pPara = TBoxPara.detectionTime.PEPSCommunicationDetectionTime;
			break;
		case BleAutoUpDateTime_ID:
			*(uint16_t *)pPara = TBoxPara.detectionTime.BleAutoUpDateTime;
			break;
		case GL_DEBUGLOG_SWITCH_ID:
			*(uint8_t *)pPara = TBoxPara.dataPara.glDebugLogSwitch;
			break;
		case TBOX_TRCUKLOADING_ID:
			*(uint8_t *)pPara = TBoxPara.dataPara.TboxTrcukLoading;
			break;
		case TBOX_LATITUDE_ID:
			*(uint32_t *)pPara = TBoxPara.dataPara.Tboxlatitude;
			break;
		case TBOX_LONGITUDE_ID:
			*(uint32_t *)pPara = TBoxPara.dataPara.Tboxlongitude;
			break;
		default:
			break;
	}
