...
void Framework::Rcv_Queue_Mcu_To_FW()
{
    ...
    {
    case ID_MCU_2_FW_PTC_SYNC_REQ:
        m_fw_syncFlg[ID_THREAD_MCU] = 1;
        break;
    }
#endif
...
void Framework::Rcv_Queue_Tsp_To_FW()
{
    ...
    if(fw_rev_msg_len > 0)
    {
        printf("[**framework.cpp->Rcv_Queue_Tsp_To_FW**]%s(%d): receive datalen : %d --- type : %d--%d\n", __func__, __LINE__, (int)fw_rev_msg_len,mFwQueueInfoRev.head.Gr,mFwQueueInfoRev.head.Id);
    ...
    switch(mFwQueueInfoRev.head.Id)
    {
    case ID_TSP_2_FW_THD_SYNC:
        m_fw_syncFlg[ID_THREAD_TSP] = 1;
        break;
    }
#endif
...
void Framework::Rcv_Queue_Pwr_To_FW()
{
    ...
    switch(mFwQueueInfoRev.head.Id)
    {
    case ID_PM_2_FW_SYNC:
        m_fw_syncFlg[ID_THREAD_PWR] = 1;
        break;
    }
#endif
}
...
void Framework::Rcv_Queue_Log_To_FW()
{
    ...
    switch(mFwQueueInfoRev.head.Id)
    {
    case ID_LOG_2_FW_SYNC:
        m_fw_syncFlg[ID_THREAD_LOG] = 1;
        break;
    }
#endif
}
...
BOOL Framework::fw_ParseMcu(uint8_t Id)
{
    ...
    case ID_MCU_2_FW_PTC_SYNC_REQ:
		fw_ParseMcu_SyncAck();
        break;
    case ID_MCU_2_FW_PTC_COMMON_ACK:
        break;
    case ID_MCU_2_TSP_PTC_REMOTE_CTL_ACK:
        fw_ParseMcu_Remoet_Ctl_Ack();
        break;
    case ID_MCU_2_OTA_PTC_UPG_DATA_REQ:
		fw_ParseMcu_Upg_Begain();		
        break;
    case ID_MCU_2_OTA_PTC_UPG_END:
        break;
    case ID_MCU_2_TSP_PTC_VEICHLE_INFO_REPORT:
        fw_ParseMcu_VecihleReprot();
        break;
    default:
        break;
    }
...
BOOL Framework::fw_ParseTsp(uint8_t Id)
{
    switch(Id)
    {
    case ID_TSP_2_MCU_PTC_REMOTE_CTL:
		fw_ParseTsp_RemoetCtl();
        break;
    case ID_TSP_2_BLE_PTC_TOKEN:
		fw_ParseTsp_Token();
        break;
    default:
        break;
...
BOOL Framework::fw_ParseBle(uint8_t Id)
{
    switch(Id)
    {
    case ID_BLE_2_TSP_REQUEST_TOKEN:
		fw_ParseBle_Token_Req();    
        break;
    case ID_BLE_2_MCU_PTC_CTLCMD:
        fw_ParseBle_Ctl_Cmd();  
        break;
    default:
        break;
...
BOOL Framework::fw_ParseOta(uint8_t Id,uint16_t len)
{
	...
	{
		case ID_OTA_2_FW_SYNC:
			FWLOG("[SUCCESS] ID_OTA_2_FW_SYNC!!\r\n");
			m_fw_syncCnt[ID_THREAD_OTA] = MCU_SYNC_CNT;
			break;
		case ID_OTA_2_MCU_PTC_UPD_BEGIN:
			FWLOG("[SUCCESS] ID_OTA_2_MCU_PTC_UPD_BEGIN!!\r\n");
...
int Framework::fw_McuSendQueue(uint8_t Id)
{
    ...
    case ID_FW_2_MCU_PTC_SYNC_ACK:
    {
		if(VPAK_PackageData(&msg_len,VPAK_HostACK_CMDID,(uint8_t*)mVPAK_Information.Buf) == RET_OK)
		{
			...
			ret_len = msgsnd(m_instance->ID_Queue_FW_To_MCU,(void *)&mFwQueueInfoSnd,msg_len+sizeof(ParaInfoHead_ST),IPC_NOWAIT);
			if (ret_len == -1)
			{
	...
	case ID_FW_2_MCU_PTC_REMOTE_CTL:
	{
		...
		FWLOG("ID_FW_2_MCU_PTC_UPG_BEGAIN\r\n");
		...
		case ID_FW_2_MCU_PTC_UPG_DATA:
			printf("fw_MCUsendQueue ID_FW_2_MCU_PTC_UPG_DATAstart\r\n");
		//memset(mFwQueueInfoSnd.Msgs,0,sizeof(mFwQueueInfoSnd.Msgs));
...
int Framework::fw_OtaSendQueue(uint8_t Id)
{
	...
	switch(Id)
	{
		...
		case ID_FW_2_OTA_UPG_MCU_RESULT:	
			FWLOG("[SUCCESS] FW RESULT MCU UPG\r\n");
			break;
	}

	ret_len = msgsnd(m_instance->ID_Queue_FW_To_OTA,(void *)&mFwQueueInfoSnd,sizeof(QueueInfo_ST)-sizeof(long),IPC_NOWAIT);
	if (ret_len < 0)
		{
...
int Framework::fw_TspSendQueue(uint8_t Id)
{
    ...
    case ID_FW_2_TSP_PTC_VEICHLE_INFO_REPORT:
    {
		if(TPAK_PackageData(&msg_len,TPAK_VEICHLEINFO_APPID,(uint8_t*)mTPAK_Information.Buf) == RET_OK)
		{
			...
			printf("\r\n");*/
			ret_len = msgsnd(m_instance->ID_Queue_FW_To_TSP,(void *)&mFwQueueInfoSnd,msg_len+sizeof(ParaInfoHead_ST),IPC_NOWAIT);
#if 0
			if (ret_len < 0)
				printf("framework.cpp->fw_ParseMcu_VecihleReprot msgsnd() write msg failed,errno=%d[%s]\n",errno,strerror(errno));
			else
		...
		if(TPAK_PackageData(&msg_len,TPAK_VEICHLEINFO_APPID,(uint8_t*)mTPAK_Information.Buf) == RET_OK)
        {
			...
			printf("\r\n");
			#endif
			ret_len = msgsnd(m_instance->ID_Queue_FW_To_BLE,(void *)&mFwQueueInfoSnd,msg_len+sizeof(ParaInfoHead_ST),IPC_NOWAIT);
    ...
    case ID_FW_2_TSP_PTC_TOKEN_REQ:
    {
		if(BPAK_PackageData(BPAK_REPROT_VECIHLE_INFORMATION,(uint8_t*)mTPAK_Information.Buf) == RET_OK)
		{
			...
			printf("\r\n");
			#endif
			ret_len = msgsnd(m_instance->ID_Queue_FW_To_TSP,(void *)&mFwQueueInfoSnd,112+sizeof(ParaInfoHead_ST),IPC_NOWAIT);
