...
void OTAWiFi::et_process(struct epoll_event *events, int number, int epoll_fd, int socketfd)
{
    int i;
    int ret;
    uint8_t buff[OTA_WIFI_BUFFER_SIZE] = {0};

	/*uint8_t *buff = (uint8_t *)malloc(OTA_WIFI_BUFFER_SIZE);
	if(buff == NULL)
	{
		EPOLL_LOG("malloc dataBuff error!");
		return;
	}*/
	//memset(buff, 0, OTA_WIFI_BUFFER_SIZE);
    ...
    for(i = 0; i < number; i++)
    {
        ...
        else if(events[i].events & EPOLLIN)
        {
            ...
            memset(buff, 0, OTA_WIFI_BUFFER_SIZE);
            int dataLen = recv(events[i].data.fd, buff, OTA_WIFI_BUFFER_SIZE, 0);
...
void *OTAWiFi::serverThread(void *arg)
{
    ...
    char buf[OTA_WIFI_BUFFER_SIZE] = {0};
    int i;
	...
	while(1)
	{
		...
		if (!fork())
		{
			while(1)
			{
				int dataLen = recv(fd, buf, OTA_WIFI_BUFFER_SIZE, 0);
				if(dataLen <= 0){
					EPOLL_LOG("dataLen < 0\n");
					break;
					close(fd);
				...
				else if(*(pos+5) == 2)
				{
					...
					if(upgradePackageSize == u_fileInfo.mcuFileSize)
					{
						...
						if(1)
						{
							...
							m_CMcu->pack_mcuUart_upgrade_data(0x8a, true, 0);
							EPOLL_LOG("###################### 33333 #####################\n");
...
#if 1
int OTAWiFi::storageFile(char *fileName, uint8_t *pData, int32_t len)
{
...
unsigned int Crc16Checktest(unsigned char* pData, uint32_t len, unsigned int sumInitCrc)
{
	//unsigned int sumInitCrc = 0xffff;
	unsigned int ui16Crc = 0;
	unsigned int i;
...
int calculate_files_CRC(char *fileName, unsigned int *crc)
{
	...
	{
		u16InitCrc = Crc16Checktest(buff, nRead, u16InitCrc);
	}
...
int check_mcu_file_is_exist()
{
	...
	CMcu::GetInstance()->mcuUpgradeMode = MCU_UPGRADE_STATE_SNED_UPGREADE_INFO;
#else
	CMcu::GetInstance()->pack_mcuUart_upgrade_data(0x8a, true, 0);
...
int check_ble_file_is_exist()
{
	...
	if( access(BLE_FILE_NAME, F_OK) )
	{
		printf("Exisist mcu upgrade file.\n");
		return -1;
		//bleUart::Get_BleInstance()->pack_bleUart_upgrade_data(0x8a, true, 0);
	}

	filesize = get_file_size(BLE_FILE_NAME);
	if(filesize == -1)
	{
		printf("get file size error!");
		memcpy(upgradeInfo.newbleVersion, "V1.0.1",strlen("V1.0.1"));
		upgradeInfo.bleVersionSize[0] = 0xff;
		upgradeInfo.bleVersionSize[1] = 0xff;
		upgradeInfo.bleVersionSize[2] = 0xff;
		upgradeInfo.bleVersionSize[3] = 0xff;

		upgradeInfo.bleVersionCrc16[0] = 0;
		upgradeInfo.bleVersionCrc16[1] = 0;
		if( !access(BLE_FILE_NAME, F_OK) )
		{
			system(RM_BLE_FILE);
			system("sync");
		...
		bleupgrade = false;
		bleUart::Get_BleInstance()->pack_bleUart_upgrade_data(0x8a, true, 1);
		printf("2222222222222222222222222222222222222222222222222222222222222\n");
		return 0;
	}
		

	calculate_files_CRC(BLE_FILE_NAME, &crc);
	...
	upgradeInfo.bleVersionSize[0] = (filesize>>24)&0xff;
	upgradeInfo.bleVersionSize[1] = (filesize>>16)&0xff;
	upgradeInfo.bleVersionSize[2] = (filesize>>8)&0xff;
	upgradeInfo.bleVersionSize[3] = (filesize>>0)&0xff;

	upgradeInfo.bleVersionCrc16[0] = 0;
	upgradeInfo.bleVersionCrc16[1] = 0;

	upgradeInfo.bleVersionCrc16[0] = (crc>>8)&0xff;
	upgradeInfo.bleVersionCrc16[1] = (crc>>0)&0xff;
