...
static qmi_idl_service_object_type  nas_service_object;
...
int nas_ind_register()
{
    ...
    nas_ind_req_msg.sig_info_valid = FALSE;
...
qmi_client_type	 cur_nas_svc_client;

int nas_init()
{
    int ret;
    if(cur_nas_svc_client != NULL)
    {
        return 0;
    }
    ...
    if(ret != 0)
    {
        printf("nas_qmi_init fail!\n");
        return -1;
    }
    cur_nas_svc_client = nas_svc_client;

    ret = nas_ind_register();
    if(ret != 0)
    {
        qmi_client_release(cur_nas_svc_client);
        cur_nas_svc_client = NULL;
...
int get_SignalStrength(int *level, int* mode)
{
    ...
    qmi_error = qmi_client_send_msg_sync(cur_nas_svc_client,
...
{    
    ...
    req_msg.request_mask = QMI_NAS_REQUEST_SIG_INFO_RSSI_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_ECIO_MASk_V01|
                         QMI_NAS_REQUEST_SIG_INFO_IO_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_SINR_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_ERROR_RATE_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_RSRQ_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_LTE_SNR_MASK_V01|
                         QMI_NAS_REQUEST_SIG_INFO_LTE_RSRP_MASK_V01;
    ...
    if(QMI_NO_ERR != resp_msg.resp.result) {   
        return -1;
    }  

    memcpy(signal_strength, &resp_msg, sizeof(nas_get_signal_strength_resp_msg_v01));
...
{    
    ...
    memcpy(serving_system, &resp_msg, sizeof(nas_get_serving_system_resp_msg_v01));
...
{    
    nas_get_system_selection_preference_req_msg_v01  req_msg;
    ...
    memset(&req_msg, 0, sizeof(nas_get_system_selection_preference_req_msg_v01));
    memset(&resp_msg, 0, sizeof(nas_get_system_selection_preference_resp_msg_v01));

    qmi_error = qmi_client_send_msg_sync(cur_nas_svc_client,
                                        QMI_NAS_GET_SYSTEM_SELECTION_PREFERENCE_REQ_MSG_V01, 
                                        &req_msg, 
                                        sizeof(nas_get_system_selection_preference_req_msg_v01), 
                                        &resp_msg, 
    ...
    printf("mode_pref_valid = %d, mode_pref = %d\n", resp_msg.mode_pref_valid,resp_msg.mode_pref);
    *mode_pref = resp_msg.mode_pref;
    return 0;
}
