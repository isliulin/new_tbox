    ...
    switch(msg_id)
    {
        case MCM_SMS_PP_IND_V01:
            ...
            {   
                ...
                #endif
                strcpy(sms_info.message_content, (const char *)utf8_msg);
            ...
            else if (pp_sms->message_format == MCM_SMS_MSG_FORMAT_TEXT_ASCII_V01) 
            {
			 ...
			 sms_info.format = pp_sms->message_format;
            strcpy(sms_info.source_address,pp_sms->source_address);
            sms_cli_ind_cb(SIMCOM_EVENT_SMS_PP_IND,&sms_info);
            break;
         default:
            MCM_SMS_TEST_LOG("=== Received a unknow message %d===", msg_id);
            break;
    }
}
...
static int mcm_sms_send(mcm_sms_msg_format_t_v01 format, char *dest_addr, unsigned char *data, int data_len)
{
    mcm_error_t_v01 mcm_error;
...
static int mcm_sms_event_register()
{
    mcm_error_t_v01 mcm_error;
...
static int mcm_sms_event_unregister()
{
    mcm_error_t_v01 mcm_error;
    mcm_sms_event_register_req_msg_v01 reg_req;
    mcm_sms_event_register_resp_msg_v01 reg_resp;
    ...
    reg_req.register_sms_pp_event_valid = FALSE;
    reg_req.register_sms_pp_event = FALSE;
    ...
    if (mcm_error == MCM_SUCCESS_V01)
    {
        if (reg_resp.response.result == MCM_RESULT_SUCCESS_V01)
        {
            MCM_SMS_TEST_LOG("unregister sms event successful");
        }
        else
        {
            MCM_SMS_TEST_LOG_ERROR("unregister sms event failure error=%d",reg_resp.response.error);
            return MCM_SMS_ERROR;
        }
    }
    else
    {
        MCM_SMS_TEST_LOG_ERROR("unregister sms event failure mcm_err=%d",mcm_error);
        return MCM_SMS_ERROR;
    }
...
static void mcm_sms_release()
{
	mcm_sms_event_unregister();
...
{
	int ucs_len;
	unsigned short ucs_data[500] = {0};

	if (smsMode < 1 || smsMode > 3)
	{
		MCM_SMS_TEST_LOG("Error: smsMode must be 1,2,3\n");
		return -1;
	}
	...
	{
		int i;
		printf("send_message: content_len=%d\n", content_len);
		codeconvert_utf82unicodeStr(content, ucs_data, &ucs_len);
		printf("send_message: ucs_len=%d\n", ucs_len);
		for(i = 0; i < ucs_len; i++)
		{
	...
	{
		return mcm_sms_send((mcm_sms_msg_format_t_v01)2, phoneNumber,content, content_len);
	}
	
}

int mcm_ril_service_start()
{
    FILE *fp = NULL;
    char buff[128] = {0};
    char ps[128] = {0};

    memcpy(ps, "ps -ef | grep mcm_ril_service | grep -v grep | wc -l",strlen("ps -ef | grep mcm_ril_service | grep -v grep | wc -l"));
    if ((fp=popen(ps,"r")) != NULL)
    {
      if (fgets(buff, 128, fp) != NULL)
      {
        if (atoi(buff) != 0)
        {
		 ...
		 {
        	system("start-stop-daemon -S -b -a /usr/bin/mcm_ril_service");
